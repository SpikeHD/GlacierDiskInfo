...
impl DiskAtaLink {
  pub fn for_disk(disk: &PathBuf) -> Result<Self, Box<dyn std::error::Error>> {
    ...
    let links = std::fs::read_dir(format!("/dev/disk/by-path"))?;

    for link in links {
      let link = link?;
      let link_path = link.path();
      let link = fs::read_link(&link_path).unwrap_or_default().to_str().unwrap_or_default().to_string();
      
      // The link should be something like ../../sdX
      if link.ends_with(format!("/{disk}").as_str()) {
        // Get the port from the links filename
        let port = link_path.file_name().unwrap_or_default().to_str().unwrap_or_default();
